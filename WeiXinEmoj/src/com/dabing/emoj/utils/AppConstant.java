...
public class AppConstant {
	...
	//加密seed
	public static final String ENCRYPT_SEED = "11228";
	...
	//常见问题
	public static final String SETTING_PROBLEM = "[{\"p\":\"为什么无法添加为微信插件?\",\"a\":\"极少情况下会出现此问题,请卸载微信后重新安装,此问题即可解决。\"},{\"p\":\"为什么有些表情发在微信上显示很小?\",\"a\":\"这和手机屏幕的像素密度有关系。在像素密度较高的屏幕上,图片显示要小一些。2.1版本中我们对一部分表情进行了手工处理,解决了此问题。但是由于表情数量太多,存在该问题的图片我们无法短时间内一一处理,后续会逐步解决,请大家谅解。\"},{\"p\":\"自定义表情如何使用?\",\"a\":\"首先将打算使用的表情图片复制到手机SD卡中(建议给表情图片新建一个文件夹),然后在'自定义表情'栏目点击'加号'添加就可以使用了。如果出现提到过的表情显示尺寸较小的问题,你也可以自己对图片进行处理。\"},{\"p\":\"为什么用了表情包以后,我一打开相册传照片就弹出来表情包呢?\",\"a\":\"这是因为您将表情包设为了打开相册的默认APP。解决办法:进入“应用程序”-“管理应用程序”-找到微信表情包,在【默认启动】分类下点击“清除默认设置”。建议您不要将微信表情包设为打开图片的默认APP。\"}]";		
	/**
	 * 图片源后缀
	 */
